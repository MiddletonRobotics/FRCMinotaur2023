...
public class FalconConfigurer {

    public static WPI_TalonFX[] leftMotors;
    public static WPI_TalonFX[] rightMotors;

    public DifferentialDrive drivetrain;

    public void setupFalconFX() {
        leftMotors = new WPI_TalonFX[Constants.leftMotorCount];
        leftMotors[0] = new WPI_TalonFX(Constants.leftMasterID);
        leftMotors[1] = new WPI_TalonFX(Constants.leftSlaveID);

        rightMotors = new WPI_TalonFX[Constants.rightMotorCount];
        rightMotors[0] = new WPI_TalonFX(Constants.rightMasterID);
        rightMotors[1] = new WPI_TalonFX(Constants.rightSlaveID);
    ...
    public void setVoltageCompensationRampRate(double rate) {
        for (int i = 0; i < 2; i++) {
            leftMotors[i].configVoltageCompSaturation(rate);
            rightMotors[i].configVoltageCompSaturation(rate);
        }
    }
